...
public final class PushUpDetector {
    ...
    private let minSecondsBetweenReps: TimeInterval = 0.35
    ...
    public func processPoseObservation(_ observation: VNHumanBodyPoseObservation) {
        do {
            ...
            let leftConf = (leftShoulder.confidence + leftElbow.confidence + leftWrist.confidence) / 3.0
            let rightConf = (rightShoulder.confidence + rightElbow.confidence + rightWrist.confidence) / 3.0

            guard leftConf > 0.15 || rightConf > 0.15 else {
                isFormValid = false
    ...
    private func updateStateMachine(elbowAngle: Double) {
        ...
        case .bottom:
            if elbowAngle > 105.0 {
                ...
                formFeedback = "Now extend your arms!"
            }
    ...
    private func registerCompletedRep() {
        ...
        guard minElbowAngleInCurrentRep <= 105.0 else {
            currentState = .top
            formFeedback = "Go lower on the next rep"
            return
        }
